...
"""
Entry point for hallucination processing.

This script provides a unified interface for running hallucination checks
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Hallucination Detection Tool",
    ...
    parser.add_argument("--original-video-path", type=str, required=True, help="Path to the original video file")
    ...
    parser.add_argument(
        "--env-file",
        type=str,
        default="checks/hallucination/.env",
...
def validate_args(args: argparse.Namespace) -> bool:
    ...
    return validate_uri(args.augmented_video_path) and validate_uri(args.original_video_path)
...
def save_results_to_file(results: Dict[str, Any], clip_id: str, output_dir: str) -> str:
    ...
    # Save to file
    output_file = output_path / f"{clip_id}.hallucination.results.json"
...
def main() -> None:
    ...
    if config is None:
        ...
        sys.exit(1)
    config = config.get("metropolis.hallucination", {})
    ...
    # Process the sample
    try:
        processor = HallucinationProcessor(params=config, verbose=args.verbose)
        result = processor.process(args.clip_id, args.original_video_path, args.augmented_video_path)
        save_results_to_file(result.model_dump(), args.clip_id, args.output_dir)
        print(
            f"\nCheck complete, results saved to: bazel-out/k8-fastbuild/bin/checks/hallucination/run.runfiles/_main/{args.output_dir}/{args.clip_id}.hallucination.results.json"
        )
        print(f"\nHallucination score: {result.score}")
        print(f"Threshold: {result.threshold}")
        print(f"Passed: {result.passed}")
        print(f"Total frames: {result.total_frames}")
        print(f"Total hallucinated pixels: {result.total_hallucinated_dynamic_pixels}")
        print(f"Total augmented dynamic pixels: {result.total_augmented_dynamic_pixels}")

...
--noexperimental_starlark_action_templates_api
...
: ${BAZEL_BUILD_MATCH_PATTERN__bin:='(.*_binary|multirun|format_multirun|oci_push|oci_load)'}
: ${BAZEL_QUERY_MATCH_PATTERN__bin:='(binary|multirun|format_multirun|alias|oci_push|oci_load)'}
: ${BAZEL_BUILD_MATCH_PATTERN_RUNTEST__bin:='(.*_(binary|test)|test_suite|multirun|format_multirun|oci_push|oci_load)'}
: ${BAZEL_QUERY_MATCH_PATTERN_RUNTEST__bin:='(binary|test|multirun|format_multirun|alias|oci_push|oci_load)'}
...
# Usage: _bazel__matching_aliases_to_runnable <prefix>
#
# Extracts alias targets from BUILD file (via stdin) that point to runnable targets.
# Only returns aliases whose 'actual' attribute contains patterns like:
# - 'binary', 'format_multirun', '_run', etc.
_bazel__matching_aliases_to_runnable() {
  local target_prefix="$1"
  local content=$(sed 's/#.*$//' | tr "\n" " ")

  # Extract each alias() block and process it
  echo "$content" | grep -o 'alias *([^)]*)' | while IFS= read -r alias_block; do
    # Extract name attribute
    local name=$(echo "$alias_block" | sed -n 's/.*name *= *['\''\"]\([^'\''\"]*\)['\''\""].*/\1/p')
    # Extract actual attribute
    local actual=$(echo "$alias_block" | sed -n 's/.*actual *= *['\''\"]\([^'\''\"]*\)['\''\""].*/\1/p')

    # Check if name starts with prefix and actual looks runnable
    # We check if actual contains common runnable patterns or if the alias name suggests it's runnable
    if [[ -n "$name" ]] && [[ -n "$actual" ]] && [[ "$name" == "$target_prefix"* ]]; then
      # Include if: actual contains runnable patterns, OR name ends with common runnable suffixes
      if [[ "$actual" =~ (binary|format_multirun|_run|_tool|_script|/bin:|/cmd:) ]] || \
         [[ "$name" =~ (binary|_run|_tool|_script|^run_|^deploy)$ ]]; then
        echo "$name"
      fi
    fi
  done
...
_bazel__expand_rules_in_package() {
  ...
  if _bazel_completion_use_query; then
    ...
    for root in $(_bazel__package_path "$workspace" "$displacement"); do
      ...
      if [ -f "$buildfile" ]; then
        # Get regular matching targets (excluding alias from pattern for grep-based approach)
        result=$(_bazel__matching_targets \
                   "$pattern" "$rule_prefix" <"$buildfile")
        # For runnable targets (label-bin), also check for aliases pointing to runnable targets
        if [[ "$label_type" == "label-bin" ]]; then
          local alias_result=$(_bazel__matching_aliases_to_runnable "$rule_prefix" <"$buildfile")
